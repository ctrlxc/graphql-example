...
クエリの構築などデータベース固有の処理がアプリケーションのコード中に散在しないようにRepositoryパターンを採用します。  
sqlboilerで自動生成されたデータ取得／更新のAPIの呼び出しは基本的には全てRepositoryを経由して行います。

```
アプリケーションコード -> Repositoryコード -> sqlboilerの自動生成コード -> データベース
```
